/***************************************************
  ...
  designed for Adafruit Feather M0 Adalogger
  also depends on:
  ds3231 featherwing
...
void setup()
{
  ...
  Serial.println("Bounce attach");

  // bounce2 setup for buttons
  button5.attach(BUTTON_5);
  button5.interval(DEBOUNCE_PIN_MS);
...
/*-----------------------------------------------------------------------------*/
// wake up from pin 6
// test for long press on button 6
// if not long press, go back to sleep
// otherwise start timer waiting for button 5
...
void doOnReady(void)
{
  ...
  // start sleep timer
  sleepTimeout = millis();
  // waut for input until timeout
  while ((millis() - sleepTimeout) < SLEEP_TIMEOUT)
  {
    if (button5.update())
    {
      if (button5.read() == LOW)
      {
        ...
        // try looping sleep/wake in the record function
...
void doRecordSleep(void)
{
  ...
  Serial.println("Detatched interrupt from pin A0");
...
void doRecord(void)
{
  ...
  while (runState != STATE_OFF)
  {

    Serial.print("doRecord");
  
    // see if button 5 is being held down
